        ...
        <div className="flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between">
          <div className="max-w-sm">
            ...
            <p className="text-sm leading-relaxed">
              ...
              {k ? (
                <>
                  {" "}
                  — turning {fmt(k.total_violations)} historical violations into
                  where to go, when, and what to do for Bengaluru Traffic Police
                  and residents.
                </>
              ) : loading ? (
          ...
          <div className="grid grid-cols-2 gap-x-10 gap-y-2 font-mono text-[0.78rem]">
            ...
            <span className="text-white">
              {k ? `${k.date_min} → ${k.date_max}` : "—"}
            </span>
            <span className="text-[#7e8aa0]">Stations</span>
            <span className="text-white">{k ? k.n_stations : "—"}</span>
            <span className="text-[#7e8aa0]">Hotspot cells</span>
            <span className="text-white">{k ? fmt(k.n_hotspots) : "—"}</span>
            <span className="text-[#7e8aa0]">Model P@20</span>
            <span className="text-white">
              {k
                ? `${k.precision_at_20.toFixed(2)} vs ${k.naive_precision_at_20.toFixed(2)} naive`
                : "—"}
            </span>
          </div>
        </div>
