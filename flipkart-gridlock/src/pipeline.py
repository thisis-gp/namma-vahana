...
FEATURES = [
    ...
    "hist_g", "hist_t", "hist_g5", "hist_g5t", "hist_nb",
]
...
def build_day48_aggregates(train_clean: pd.DataFrame) -> dict:
    ...
    d48 = train_clean[train_clean["day"] == 48].copy()
    d48["_g5t"] = d48["g5"] + "|" + d48["timestamp"]
    agg = {
        ...
        "g5t_sum": d48.groupby("_g5t")["demand"].sum(),
        "g5t_cnt": d48.groupby("_g5t")["demand"].count(),
    }
    agg["g_mean"] = agg["g_sum"] / agg["g_cnt"]
    agg["t_mean"] = agg["t_sum"] / agg["t_cnt"]
    agg["g5_mean"] = agg["g5_sum"] / agg["g5_cnt"]
    agg["g5t_mean"] = agg["g5t_sum"] / agg["g5t_cnt"]
    return agg
...
def add_hist_features(df: pd.DataFrame, agg: dict, is_day48_train: bool) -> pd.DataFrame:
    """Attach hist_g, hist_t, hist_g5, hist_g5t. LOO for Day-48 training rows;
    plain Day-48 means for Day-49/test rows."""
    df = df.copy()
    gm = agg["global_mean"]
    g5t_key = df["g5"] + "|" + df["timestamp"]
    if is_day48_train:
        ...
        df["hist_g5t"] = _loo_mean(g5t_key, d, agg["g5t_sum"], agg["g5t_cnt"], gm)
    else:
        ...
        df["hist_g5t"] = g5t_key.map(agg["g5t_mean"]).fillna(df["hist_g5"]).fillna(gm)
    return df
