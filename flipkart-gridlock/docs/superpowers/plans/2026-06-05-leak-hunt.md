# Leak Hunt — can the test `demand` be reconstructed exactly?

> **For the implementing agent (Cursor):** This is an INVESTIGATION, not a model change. Build `leak_hunt.py` exactly as below, run it, and report **all printed output verbatim** plus your interpretation of each section. Do not modify the existing pipeline/model. Do not submit anything.

## Why
Honest modeling caps at R² ≈ 0.89 here: at the (geohash, timestamp, weather) level ~21% of `demand` variance is unexplained by ANY provided feature (it persists even with identical weather and doesn't track temperature). Competitors at score 100 (R² = 1.0) must be **reconstructing the target**, not modeling it. This script hunts for that reconstruction path. A real leak shows up as: a key that *deterministically* maps to demand in train AND covers many test rows, or the unexplained residual correlating with an artifact (Index, row order, temperature decimals).

## Task: build and run `leak_hunt.py`

- [ ] **Step 1: Create `leak_hunt.py`**
...
python leak_hunt.py
```
Expected: five labeled sections (A–E) each printing numbers + a VERDICT line. Runtime ~1–2 min.

- [ ] **Step 3: Report back** — paste the ENTIRE output verbatim, then for each section say whether the VERDICT condition triggered. Flag anything anomalous (a near-perfect correlation, det_frac≈1 with high coverage, residual R² far above 0, identical train/test rows). Do NOT act on findings yet — report them so we decide together.

## What a "hit" looks like (so you know when we've found it)
- **A:** a key with `det_frac ≈ 1.0` and large `test_cov` → copy demand by that key → instant high score.
- **B:** `corr ≈ ±1` or in-sample R² ≈ 1.0 → demand is encoded in Temperature; invert it.
- **C/D:** demand or its residual correlates with `Index`, `Index % n`, row position, or temperature decimals → the target leaked through ordering/encoding.
- **E:** identical train/test rows → direct copy; a completable grid → the generator may be reversible.

If every section comes back clean (verdicts all negative, residual R² ≈ 0), then there is no recoverable leak in the provided files, the 100s are using something external/undisclosed, and ~0.89 is the honest ceiling — we stop chasing 100.
